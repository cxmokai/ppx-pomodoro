...
export const useTimer = () => {
  ...
  const getModeDuration = useCallback((currentMode: TimerMode): number => {
    switch (currentMode) {
      case 'work':
  ...
  const skipMode = useCallback(() => {
    ...
    const newMode = mode === 'work' ? (sessionCount + 1) % settings.longBreakInterval === 0 ? 'longBreak' : 'shortBreak' : 'work'
